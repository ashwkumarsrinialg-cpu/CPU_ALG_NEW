 ...
 *   - 15 independent random seeds
 *   - 30 processes per epoch
 *   - 3 workload profiles: CPU-bound, I/O-bound, Mixed
 *   - 3 features: X1 (last burst), X2 (historical mean), X3 (queue depth)
 *   - 5 algorithms: FCFS, RR, SJF, SRTF, MLR+SGD
 *   - Reports mean ± std AWT, ATAT, context switches per algorithm
 *   - Paired t-test (MLR+SGD vs SJF) per workload type
 *
 * Compile: gcc -O2 -lm -o scheduler scheduler.c
 * Run:     ./scheduler
 */
